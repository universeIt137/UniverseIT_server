const express = require("express");
const router = express.Router();
const { ObjectId } = require("mongodb");
const { getCollection } = require("../config/db");

const representativeCollection = getCollection("representative");

router.post("/representative", async (req, res) => {
  const data = req.body;
  const result = await representativeCollection.insertOne(data);
  res.send(result);
});

router.get("/representative", async (req, res) => {
  const result = await representativeCollection.find().toArray();
  res.send(result);
});

router.get("/representative/:id", async (req, res) => {
  const id = req.params.id;
  const query = { _id: new ObjectId(id) };
  const result = await representativeCollection.findOne(query);
  res.send(result);
});

router.put("/representative/:id", async (req, res) => {
  const data = req.body;
  const id = req.params.id;
  const query = { _id: new ObjectId(id) };
  const options = { upsert: true };
  const updatedInfo = {
    $set: {
      ...data,
    },
  };
  const result = await representativeCollection.updateOne(
    query,
    updatedInfo,
    options,
  );
  res.send(result);
});

router.delete("/representative/:id", async (req, res) => {
  const id = req.params.id;
  const query = { _id: new ObjectId(id) };
  const result = await representativeCollection.deleteOne(query);
  res.send(result);
});

module.exports = router;